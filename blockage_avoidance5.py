...
class minicar_f2:
    ...
    def update_control(self):
        ...
        if control < -0.5:
            self.set_angvel = -int(np.around(2 * 3.14 * control * TURN_RIGHT_PARAM))
        elif control > 0.5:
            self.set_angvel = -int(np.around(2 * 3.14 * control * TURN_LEFT_PARAM))
        else:
            self.set_angvel = 0
    ...
    def rl_update(self, RL, lidar, epoch):
        lidar_data = lidar.scan()
        this_state0 = np.append(np.array([self.f1f2_distance, self.f1f2_distance]), lidar_data)
        #this_state = np.append(np.array([self.f1f2_distance, self.f1f2_distance - self.state[0]]), lidar_data)
        this_state = this_state0 / np.mean(this_state0)
        #print(this_state.shape)
        print(this_state0)
        print('mean',meandis(this_state0))
        if (self.state != STATEMAX).any():
            RL.store_transition(self.state, np.eye(5)[self.action + 2], self.reward)
            if epoch % update_period == 0:
                RL.learn()
            self.action = RL.choose_action(this_state)
            print('action',self.action)
            #self.reward = np.random.randint(-1,2)
            if this_state0[0] < d_threshold:
                self.reward = r0 - c0 - 2 ** ((d0 - meandis(this_state0)) / 100)
            else:
                self.reward = r0 - 2 ** ((d0 - meandis(this_state0)) / 100)
        #(self.state[0] < pre_distance) * self.state[0] * (abs(self.action) - action_threshold) * 1e6
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # 接收距离信息
            # 2. update rl
            minicar.rl_update(RL, lidar, epoch)
            # 更新RL网络，作出决策
            # 3. update control
